                                ...
                                <ul class="text-nowrap" id="images">
                                    @foreach ($productImages as $productImage)
                                        <li class="float-left d-inline-block mr-2 mb-2"
                                            style="position: relative; width: 24%;">
                                            <form action="{{ route('image.destroy',[$product->id, $productImage->id]) }}" method="post">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                            ...
                                            <div style="width: 100%; height: 300px; overflow: hidden;">
                                                <img src="{{ asset('front/images/products/'. $productImage->path) }}"
                                                    alt="Image">
                                            </div>
                                        </li>
                                    @endforeach

                                    <li class="float-left d-inline-block mr-2 mb-2" style="width: 24%;">
                                        <form method="post" action="{{ route('image.store', $product->id) }}" enctype="multipart/form-data">
                                            @csrf
